...
class Processor:
    def __init__(self, registers=[0, 0, 0, 0, 0, 0]):
        ...
        self.initializedRegisters = registers
    ...
    def reset(self):
        self.instructionPointer = 0
        self.registers = self.initializedRegisters
    ...
    def start(self):
        ...
        while self.instructionPointer >= 0 and self.instructionPointer < len(self.program):
            ...
            self.totalInstructions += 1
            if self.instructionPointer == 28:
                print(self.registers[3])
                if self.registers[3] in r3Values:
                    print(r3Values[-1])
                else:
                    r3Values.append(self.registers[3])
...
def a21(fileName):
    processor = Processor(registers=[0, 0, 0, 0, 0, 0])
    ...
    processor.setPrinting(False)
    processor.start()
    print(processor.registers)
    print(processor.totalInstructions)
